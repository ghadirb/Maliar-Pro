...
## بازاریار: راه‌اندازی امن منبع قیمت

اندروید هیچ‌وقت مستقیماً ترب، دیجی‌کالا یا کانال تلگرام را واکشی نمی‌کند - همه از
مسیر `marketSearch` در `Code.gs` عبور می‌کنند (نتیجه هم ۱۵ دقیقه Cache می‌شود):

- **ترب و دیجی‌کالا (فقط بخش خرده):** `torobSearch_` و `digikalaSearch_` از endpoint
  عمومی JSON خودِ این دو سایت استفاده می‌کنند (بدون کلید/لاگین). این endpointها
  رسمی/مستند نیستند و ممکن است هر زمان تغییر کنند یا IP سرورهای Google Apps Script
  را موقتاً محدود کنند - در این حالت آن ارائه‌دهنده فقط نتیجه‌ی خالی برمی‌گرداند و
  بقیه‌ی نتایج (کانال‌های تلگرام کاربر) طبیعی کار می‌کنند.
- **کانال‌های تلگرام (هم عمده، هم خرده):** فقط کانال‌هایی که خودِ کاربر در «منابع
  عمده و خرده» با آدرس `t.me/channel` ثبت کرده، از طریق صفحه‌ی عمومی
  `https://t.me/s/<channel>` (همان چیزی که مرورگر بدون لاگین می‌بیند) خوانده و برای
  عبارت جست‌وجو فیلتر می‌شوند؛ هیچ کانال یا بات دیگری به‌صورت خودکار خوانده نمی‌شود.
- استخراج قیمت از متن پیام‌ها (`extractPriceInfo_`) بین این آداپتر تلگرام و ثبت
  دستیِ «پیام فروشنده» در اپ مشترک است.
- افزودن یک ارائه‌دهنده‌ی جدید فقط یعنی یک تابع تازه در `marketProviders_` اضافه
  کنید که همان قرارداد `[{name, price, minPrice, maxPrice, url, source, confidence}]`
  را برگرداند.
