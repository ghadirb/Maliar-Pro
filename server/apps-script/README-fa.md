...
اندروید هیچ‌وقت مستقیماً ترب، دیجی‌کالا یا تلگرام را واکشی نمی‌کند. مسیرهای
`marketSearch` و `marketParseMessage` در `Code.gs` محل واحد آداپترها هستند و نتیجهٔ
جستجو ۱۵ دقیقه Cache می‌شود. در تابع `marketProviders_` فقط آداپترهایی را اضافه کنید
که API، فید یا اجازهٔ کتبی معتبر دارند؛ اسکرپ HTML فروشگاه‌ها یا خواندن کانال خصوصی
تلگرام در این پروژه پیاده نشده است. برای کانال‌ها، کاربر می‌تواند متن پیام را در اپ
وارد کند و قیمت‌های تشخیص‌داده‌شده محلی ثبت می‌شوند.
