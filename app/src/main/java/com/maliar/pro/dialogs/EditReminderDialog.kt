...
class EditReminderDialog(
    ...
    fun show() {
        ...
        categorySpinner.setSelection(categories.indexOf(reminder.category).let { if (it >= 0) it else 0 })
        val repeatKeys = REPEAT_PATTERN_LABELS_EDIT.keys.toList()
        repeatPatternSpinner.adapter = ArrayAdapter(context, android.R.layout.simple_spinner_dropdown_item, REPEAT_PATTERN_LABELS_EDIT.values.toList())
        ...
        titleInput.setText(reminder.title); descriptionInput.setText(reminder.description)
        ...
        timeButton.setOnClickListener {
            com.maliar.pro.ui.common.MaliarTimePickerDialog.show(context, hour, minute) { h, min -> hour = h; minute = min; refreshTimeButtonText() }
        }

        AlertDialog.Builder(context).setTitle("ویرایش یادآوری").setView(view)
            .setPositiveButton("ذخیره") { _, _ ->
                val title = titleInput.text?.toString().orEmpty(); if (title.isBlank()) return@setPositiveButton
                val priority = when (priorityChipGroup.checkedChipId) {
                    R.id.chipLowPriority -> Priority.LOW; R.id.chipHighPriority -> Priority.HIGH; else -> Priority.MEDIUM
                }
                val alertType = when (alertTypeChipGroup.checkedChipId) {
                    R.id.chipAlertFullScreen -> AlertType.FULL_SCREEN; R.id.chipAlertSmart -> AlertType.SMART; else -> AlertType.NOTIFICATION
                }
                ...
                val cal = Calendar.getInstance().apply { timeInMillis = dateMillis; set(Calendar.HOUR_OF_DAY, hour); set(Calendar.MINUTE, minute); set(Calendar.SECOND, 0) }
                val updated = reminder.copy(
                    title = title, description = descriptionInput.text?.toString().orEmpty(), priority = priority.name,
                    alertType = alertType.name, triggerTime = cal.timeInMillis, repeatPattern = repeatPattern.name,
                    category = categorySpinner.selectedItem?.toString().orEmpty()
                )
                CoroutineScope(Dispatchers.Main).launch { smartReminderManager.updateReminder(updated); onSaved() }
            }
