...
class FinancialInsightsFragment : Fragment() {
    ...
    override fun onViewCreated(v: View, s: Bundle?) {
        ...
        lifecycleScope.launch {
            combine(accounting.getAllExpenses(), accounting.getAllIncomes(), financial.getAllDebts()) { e, i, d -> Triple(e, i, d) }
                .collect { (expenses, incomes, debts) -> render(expenses, incomes, debts) }
        }
    }
    private fun render(expenses: List<com.maliar.pro.database.Expense>, incomes: List<com.maliar.pro.database.Income>, debts: List<com.maliar.pro.database.Debt>) {
        ...
        if (current.size < 2) add("دادهٔ کافی نیست", "برای تحلیل قابل‌اتکا حداقل چند هزینه در دورهٔ جاری لازم است.", "برچسب: بر اساس داده‌های ثبت‌شده")
            ...
            current.groupBy { it.category.ifBlank { "عمومی" } }.maxByOrNull { it.value.sumOf { x -> x.amount } }?.let { (category, rows) ->
                add("بیشترین دستهٔ هزینه", "دستهٔ «$category» با ${CurrencyFormatter.format(rows.sumOf { it.amount })} بیشترین سهم را دارد.", "برچسب: پیشنهاد · ${rows.size} تراکنش مرتبط")
            }
            val avg = current.sumOf { it.amount } / current.size
            current.maxByOrNull { it.amount }?.takeIf { it.amount > avg * 2 }?.let { add("هزینهٔ غیرعادی", "هزینهٔ «${it.description.ifBlank { it.category }}» بیش از دو برابر میانگین این دوره است؛ بررسی آن پیشنهاد می‌شود.", "برچسب: پیشنهاد · تراکنش مرتبط") }
        }
    ...
    private fun add(h: String, body: String, tag: String) { box.addView(LinearLayout(requireContext()).apply { orientation = LinearLayout.VERTICAL; setPadding(16, 14, 16, 14); addView(title(h).apply { textSize = 16f }); addView(note(body)); addView(note(tag)) }) }
